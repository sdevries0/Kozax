...
def get_data(key, env, batch_size, dt, T, param_setting):
    ...
    noise_keys = jr.split(noise_key2, batch_size)
...
dynamic memory and the dynamic environment. At every time step, the state of the environment is mapped to observations and the latent memory is mapped to a control signal. Afterwards, 
...
class Evaluator:
    ...
    def evaluate_trajectory(self, candidate: Array, x0: Array, ts: Array, target: float, noise_key: jr.PRNGKey, params: Tuple, tree_evaluator: Callable) -> Tuple[Array, Array, Array, Array, float]:
        ...
        sol = diffrax.diffeqsolve(
            ...
            args=(env, state_equation, readout, noise_key, target, tree_evaluator), stepsize_controller=self.stepsize_controller, throw=False
        )

        xs = sol.ys[:,:self.latent_size]
        # Get observations of the state at every time step
        _, ys = jax.lax.scan(env.f_obs, noise_key, (ts, xs))
    ...
    def _drift(self, t, x_a, args):
        env, state_equation, readout, noise_key, target, tree_evaluator = args
        x = x_a[:self.latent_size]
        a = x_a[self.latent_size:]

        _, y = env.f_obs(noise_key, (t, x)) #Get observations from system
...
if __name__ == "__main__":
    ...
    operator_list = [
        ...
        {"string": "*", "fn": lambda x, y: x * y, "arity": 2, "prob": 0.5, "flops": 1},
        {"string": "sin", "fn": lambda x: jnp.sin(x), "arity": 1, "prob": 0.1, "flops": 1},
        {"string": "cos", "fn": lambda x: jnp.cos(x), "arity": 1, "prob": 0.1, "flops": 1}
    ]
