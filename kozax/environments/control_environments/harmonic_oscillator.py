...
class ChangingHarmonicOscillator(EnvironmentBase):
    ...
    def sample_params(self, batch_size, mode, ts, key):
        ...
        if mode == "Constant":
            ...
            omega_decay_factors = jrandom.uniform(args_key, shape=(batch_size,), minval=0.99, maxval=1.01)
            zeta_decay_factors = jrandom.uniform(args_key, shape=(batch_size,), minval=0.99, maxval=1.01)
            init_omegas = jrandom.uniform(omega_key, shape=(batch_size,), minval=0.5, maxval=1.5)
            init_zetas = jrandom.uniform(zeta_key, shape=(batch_size,), minval=0.3, maxval=0.7)
