...
def add_subtree(tree: Array, 
    ...
    tree_indices = jnp.tile(jnp.arange(max_nodes)[:, None], reps=(1, 5))
    ...
    is_leaf = jnp.isin(node_ids, variable_indices)
    mutate_idx = jr.choice(select_key, jnp.arange(tree.shape[0]), p=is_leaf * 1.0)
    subtree = sample_tree(sample_key, jnp.minimum(max_init_depth, 2), variable_array, mutate_idx == (max_nodes-1))
    subtree_size = jnp.sum(subtree[:, 0] != 0)
    remaining_size = mutate_idx - jnp.sum(tree[:, 0] == 0)
    
    # Create new tree
    child = jnp.tile(jnp.array([0.0, -1.0, -1.0, 0.0, 0.0]), (max_nodes, 1))
...
def mutate_leaf(tree: Array, 
    ...
    child = jax.lax.select(new_leaf == 1, child.at[mutate_idx, 4].set(coefficient), child.at[mutate_idx, 4].set(0))
    return child
...
def replace_with_one_subtree(tree: Array, 
    ...
    subtree = sample_tree(key, 2, variable_array, False)
    subtree_size = jnp.sum(subtree[:, 0] != 0)
...
def replace_with_two_subtrees(tree: Array, 
    ...
    _, _, end_idx = jax.lax.while_loop(lambda carry: carry[1] > 0, find_end_idx, (tree, 1, mutate_idx))
    remaining_size = end_idx - jnp.sum(tree[:, 0] == 0) + 1
    subtree1 = sample_tree(key1, 1, variable_array, False)
    subtree1_size = jnp.sum(subtree1[:, 0] != 0)
    subtree2 = sample_tree(key2, 1, variable_array, False)
...
def delete_operator(tree: Array, 
    ...
    child = jax.lax.select(new_leaf == 1, child.at[delete_idx, 4].set(coefficient), child.at[delete_idx, 4].set(0))  # Set coefficient value
...
def insert_operator(tree: Array, 
    ...
    subtree = sample_tree(subtree_key, 2, variable_array, False)  # Sample new subtree
    subtree_size = jnp.sum(subtree[:, 0] != 0)
    ...
    child_2_branches = jnp.where((tree_indices <= mutate_idx - 1) & (tree_indices >= mutate_idx - subtree_size - tree_size), lower_tree, child)  # Insert subtrees in new tree
    ...
    child = jax.lax.select(mutate_idx == (max_nodes-1), child.at[-1,3].set(tree[-1,3]), child)
    child = jax.lax.select(mutate_idx == (max_nodes-1), child.at[max_nodes - (new_slots - 1) * second_branch * subtree_size - 2,3].set(0.0), child)

    return child

def mutate_output_node(tree: Array, 
                    key: PRNGKey, 
                    variable_array: Array, 
                    args: Tuple):
    """Mutates the output reference of a node.
...
def get_mutations(tree: Array, 
    ...
    # mutation_probs = jnp.array([1.,1.,1.,1.,1.,1.])
    # mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] == 0) < 8, jnp.array([0., 1., 1., 1., 0., 1.]), mutation_probs)  # Tree is too big to add more nodes
    # mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] != 0) <= 3, jnp.array([1., 1., 1., 0., 0., 1.]), mutation_probs)  # Tree does not have enough operators
    # mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] != 0) == 1, jnp.array([1., 1., 0., 0., 0., 1.]), mutation_probs)  # Tree does not have operators
    mutation_probs = jnp.array([1.,1.,1.,1.,1.,1.])
    mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] == 0) < 8, jnp.array([0., 1., 1., 1., 0., 1.]), mutation_probs)  # Tree is too big to add more nodes
    mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] != 0) <= 3, jnp.array([1., 1., 1., 0., 1., 1.]), mutation_probs)  # Tree does not have enough operators
    mutation_probs = jax.lax.select(jnp.sum(tree[:, 0] != 0) == 1, jnp.array([1., 1., 0., 0., 1., 1.]), mutation_probs)  # Tree does not have operators
    mutation_probs = mutation_probs.at[2].set(mutation_probs[2] * (len(operator_indices)>1))
    mutation_probs = mutation_probs.at[5].set(num_ouputs>1)

    return jr.choice(key, jnp.arange(len(MUTATE_FUNCTIONS)), p=mutation_probs)

# Define list with possible mutation functions
MUTATE_FUNCTIONS = [add_subtree, mutate_leaf, mutate_operator, delete_operator, insert_operator, mutate_output_node]
...
def initialize_mutation_functions(mutate_args: Tuple) -> Callable:
    ...
    def mutate_trees(trees: Array, 
        ...
        mutate_functions = jax.vmap(get_mutations, in_axes=[0,0,None,None])(trees, keys, operator_indices, num_outputs)
